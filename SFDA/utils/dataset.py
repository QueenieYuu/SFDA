...
class Prostate(Dataset):
    def __init__(self, site, base_path=None, split='train', transform=None):
        ...
        if(split=='train'):
            self.images, self.labels, self.names = images[:int(trainlen)], labels[:int(trainlen)], names[:int(trainlen)]

        elif(split=='val'):
            self.images, self.labels, self.names = images[int(trainlen):int(trainlen + vallen)], labels[int(trainlen):int(trainlen + vallen)], names[int(trainlen):int(trainlen + vallen)]
        else:
            self.images, self.labels, self.names = images[int(trainlen + vallen):], labels[int(trainlen + vallen):], names[int(trainlen + vallen):]
