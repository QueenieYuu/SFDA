...
def train_perturbation(args, model, data_loader, optimizer, loss_fun, device):
    ...
    for step, (data, target, _) in enumerate(data_loader):
        optimizer.zero_grad()
...
if __name__ == '__main__':
    ...
    SAVE_PATH = os.path.join(args.save_path, 'Site_')
    ...
    for a_iter in range(start_iter, args.iters):
        ...
        with torch.no_grad():
            ...
            for client_idx, model in enumerate(models):
                val_loss, val_acc = test(args, model, val_loaders[client_idx], loss_fun, device)
            ...
            if best_changed:
                ...
                for client_idx, model in enumerate(models):
                    model_dicts = {'server_model': model.state_dict(),
                    ...
                    model_dicts['optim_{}'.format(client_idx)] = optimizers[client_idx].state_dict()

                    torch.save(model_dicts, SAVE_PATH + str(datasets[client_idx]))
                    torch.save(model_dicts, SAVE_PATH + str(datasets[client_idx]) + '_latest')
                best_changed = False
            else:
