...
def train(args, target_model, models, data_loader, optimizer, loss_fun, device):
    ...
    for step, (data, target, name) in enumerate(data_loader):
        ...
        # loss = loss_seg.mean() + torch.mean(loss_consis)
        # print('Seg Loss: {:.4f} | Consis Loss: {:.4f}'.format(loss_seg.mean(), torch.mean(loss_consis)))
        #
        loss = torch.mean(loss_consis)
        print('Consis Loss: {:.4f}'.format(torch.mean(loss_consis)))
...
if __name__ == '__main__':
    ...
    sites = ['ISBI', 'I2CVB', 'HK', 'BIDMC', 'ISBI_1.5']
    ...
    if args.test:
        ...
        test_path = '../checkpoints/prostate/target_model_best'
        checkpoint = torch.load(test_path, map_location=device)
        target_model.load_state_dict(checkpoint['server_model'], strict=False)
